...
If `contextMenu.xml` doesn't exist in the user configuration folder, Notepad++ will copy `contextMenu.model.xml` from the installation folder (if it exists), or will fall back to creating a new default `contextMenu.xml` based on internal default values.
...
If `shortcuts.xml` doesn't exist in the user configuration folder, Notepad++ will copy `shortcuts.model.xml` from the installation folder (if it exists), or will fall back to creating a new default `shortcuts.xml` based on internal default values.
...
The `<WordsStyle>` `fontStyle` attribute encodes the setting of the **Bold**, **Italic**, and **Underline** checkboxes from the **Styler** dialog, using the sum of **Bold**=1, **Italic**=2, and **Underline**=4 (so something with all three checkboxes set would have `fontStyle="7"`).
...
- `disableNppAutoUpdate.xml`: This is a zero-byte file that is used as an indicator to Notepad++ to disable auto-update, even when WinGUp (`gup.exe`) is still present.  (added v8.9.3)
    ...
    - This replaces the old advice to delete `gup.exe` to stop automatic updates, which had the side effect of breaking Plugins Admin.
...
- `shortcuts.model.xml`: If it exists, Notepad++ will use the `shortcuts.model.xml` from the installation directory as the default `shortcuts.xml` when creating a new per-user setup; if the `shortcuts.model.xml` does not exist in the installation directory, Notepad++ has a set of default internal values that it will use to populate the per-user `shortcuts.xml`.  This is useful if you have multiple users on the same PC, and want them all to start with the same keyboard shortcuts, macros, and user-defined commands when they first use Notepad++; it can also be used if you want to bundle your own customized Notepad++ installation directory to distribute to multiple PCs, then when each user first uses Notepad++ on the new computer, they will start with those values in their user `shortcuts.xml`.
