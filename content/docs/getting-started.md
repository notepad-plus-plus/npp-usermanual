...
Notepad++ had issues with the code-signing certificate that Microsoft encourages applications to have.  If you are using v8.8.2 - v8.8.9, it is recommended that you immediately upgrade to the most recent version.  However, for details on the code signing and certificates during that period, you can see this page on the [historical self-signed certificates](../certs/self-signed/).
