...
3. (Keyboard or Mouse)  Put [caret](../editing/#caret-and-cursor "typing/insertion cursor") at desired start of column block position, then execute the **Begin/End Select in Column Mode** command; move [caret](../editing/#caret-and-cursor "typing/insertion cursor") to desired end of column block position, then execute the **Begin/End Select in Column Mode** command again.
...
Multi-Editing mode (available via mouse usage only) allows you to make multiple [caret](../editing/#caret-and-cursor "typing/insertion cursor")s by using <kbd>Ctrl+Click</kbd> for each additional [caret](../editing/#caret-and-cursor "typing/insertion cursor").  This allows performing the same editing actions (typing, copy/cut/paste/delete, arrowing through the text) in multiple locations, even if they aren't lined up in a nice column, or even if there are lines between the [caret](../editing/#caret-and-cursor "typing/insertion cursor")s that you don't want to affect.  You may place as many additional [caret](../editing/#caret-and-cursor "typing/insertion cursor")s as you want.
...
#### Step 4
Type <kbd>DELETE</kbd> twice and then <kbd>ENTER</kbd> once, then <kbd>ArrowUp</kbd> to move all [caret](../editing/#caret-and-cursor "typing/insertion cursor")s up.
...
#### Step 6
Use <kbd>Ctrl+ArrowRight</kbd> to move the [caret](../editing/#caret-and-cursor "typing/insertion cursor")s to the end of the words.
...
#### Step 8
Put your [caret](../editing/#caret-and-cursor "typing/insertion cursor") after `obj.` of the 1st line, the <kbd>Alt+Shift+ArrowDown</kbd> to the last line.
(Or you can use <kbd>Ctrl+ArrowLeft</kbd> to move [caret](../editing/#caret-and-cursor "typing/insertion cursor")s after `obj.` column.)
...
#### Step 10
Move all [caret](../editing/#caret-and-cursor "typing/insertion cursor")s to right by using <kbd>ArrowRight</kbd> to between `""`, then paste (<kbd>Ctrl+V</kbd>).
...
* **Line Operations >** ⇒ submenu with actions that typically work on lines (also known as "rows") of your document
    ...
    * There are commands for changing the order of existing lines:
        * **Move Up Current Line**: will swap the current line with the line above it, effectively moving the line of the [caret](../editing/#caret-and-cursor "typing/insertion cursor") up one row in the document; if a selection spanning lines is active upon invocation, it will move those lines touched by the selection up as a group
        * **Move Down Current Line**: will swap the current line with the line below it, effectively moving the line of the [caret](../editing/#caret-and-cursor "typing/insertion cursor") down one row in the document; if a selection spanning lines is active upon invocation, it will move those lines touched by the selection down as a group
...
* **On Selection >** ⇒ submenu with actions that use the currently-selected text:
    - **Open File** ⇒ Use the selected text as a filename to open
    - **Open Containing Folder in Explorer** ⇒ If the selected text is a filename, then it will use Windows Explorer to open the folder that contains that file.  If the selected text is a directory name, it will use Windows Explorer to open the folder that contains that directory.
    - **Redact Selection █ (Shift: ●)** ⇒ Clicking this action normally will replace each character in the selection with `█`.  Clicking this action while holding <kbd>Shift</kbd> will replace each character in the selection with `●`.  (New to v8.9.2.)
    - **Search on Internet** ⇒ Use the selected text as the search term in your configured [**Settings > Preferences > Search Engine**](../preferences/#search-engine).
    - **Change Search Engine...** ⇒ Takes you to [**Settings > Preferences > Search Engine**](../preferences/#search-engine) so you can change your preferred search engine.
    - Custom commands using the current selection can be added to the **Run** menu, using the [`<UserDefinedCommands>` section of `shortcuts.xml`](../config-files/#userdefinedcommands) by using `$(CURRENT_WORD)` in the Run-command.
* **Multi-Select All >** ⇒ submenu with actions that work with the current selection or word under the [caret](../editing/#caret-and-cursor "typing/insertion cursor")
    - **Ignore Case & Whole Word** ⇒ If nothing is selected, it will determine the current word under the [caret](../editing/#caret-and-cursor "typing/insertion cursor"), and do a Multi-Select which finds all matches which match that string regardless of case, and regardless of whether the other matches are a whole word or not.  If a word or string is selected, it will Multi-Select all the strings which match the current selection.
    - **Match Case Only** ⇒ If nothing is selected, it will determine the current word under the [caret](../editing/#caret-and-cursor "typing/insertion cursor"), and do a Multi-Select which finds all matches of that word, as long as the case exactly matches.  If a word or substring is selected, it will Multi-Select all the strings which match that selection, paying attention to case.
    - **Whole Word Only** ⇒ If nothing is selected, it will determine the current word under the [caret](../editing/#caret-and-cursor "typing/insertion cursor"), and do a Multi-Select which finds all whole-word matches, regardless of case.  If a word is selected, it will Multi-Select all the whole-words which match that selection, ignoring case.
    - **Match Case & Whole Word** ⇒ If nothing is selected, it will determine the current word under the [caret](../editing/#caret-and-cursor "typing/insertion cursor"), and do a Multi-Select which finds all whole-word matches which match that string, including case.  If a word is selected, it will do a Multi-Select which finds all whole-word matches which match that string, including case.
...
- "caret" = The typing cursor, which is the text insertion point; see "[Caret and Cursor](../editing/#caret-and-cursor)".
