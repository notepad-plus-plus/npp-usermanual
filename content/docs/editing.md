...
* `Line Operations >` ⇒ submenu with actions that typically work on lines (also known as "rows") of your document
    ...
    * There are a variety of sorting algorithms:
        ...
        * `Reverse Line Order` means that the selected lines will be ordered reversely (i.e. flipped) based on their existing order, not determined by the characters or values on the line (added in v7.9.6)
