...
* `Clear Read-Only Flag` ⇒ clears the Windows Operating System (OS) read-only attribute on a file.  
    ...
    * You cannot set the OS read-only flag using this menu in Notepad++; it has to be done through the OS (though Notepad++ scripting plugins are able to ask the OS to set the OS read-only flag on the file).
