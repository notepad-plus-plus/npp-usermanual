...
- `Set Read-Only` ⇒ toggles Notepad++'s read-only flag on the active file buffer.
  ...
  - _Note_: this toggle does _not_ affect the Windows Operating System's read-only attribute on the file; if Windows has marked this file as read-only, this menu entry will be greyed out and you cannot toggle it by clicking on it. See the `Clear Read-only Flag` (below) for more on the OS flag.
