...
3. Current Position of the [caret](../editing/#caret-and-cursor "typing/insertion cursor"):
    - `Ln : ℕ`: Indicates the [caret](../editing/#caret-and-cursor "typing/insertion cursor") is on Line `ℕ`. (Line 1 is the start of the document).
    - `Col : ℕ`: Indicates the [caret](../editing/#caret-and-cursor "typing/insertion cursor") is on Column `ℕ` of the current Line. (Column 1 indicates the caret is at the start of the line.)
    - `Pos : ℕ`: When there is no active selection, this sub-field indicates which byte of the file the [caret](../editing/#caret-and-cursor "typing/insertion cursor") is on.  (1 indicates the caret is before the first byte in the document.  In many encodings, some or all characters may take up multiple bytes; there is more discussion on bytes-vs-characters in the description of the [**Go to...**](../searching/#other-search-menu-commands) command.)
