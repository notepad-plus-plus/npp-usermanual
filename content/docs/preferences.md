...
* **Tab settings**:
    ...
    * `Tab size : ___`: sets the width of the tab stop.  If the text uses literal TAB characters (`Replace by space` is turned off), there will be one TAB character in the file, but Notepad++ will _display_ that character as a gap with a width of up to that many space characters.  If the text is replacing TABs with spaces, enough literal spaces will be inserted into the file so the next character will start at the beginning of the next tab stop.
