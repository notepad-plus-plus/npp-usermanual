...
* **Replace All**: With **☑ Wrap around** checked, it makes one pass through the active document, from the very top to the very bottom, and replaces all occurrences found.  With **☐ Wrap around** unchecked, it searches from the [caret](../editing/#caret-and-cursor "typing/insertion cursor") to the end of the file (if **☐ Backward direction** is unchecked) or from the beginning of the file to the [caret](../editing/#caret-and-cursor "typing/insertion cursor") (if **☑ Backward direction** is checked) and replaces all occurrences found in that region.
    ...
    * To clarify the **Replace All** results, depending on the condition of the various settings:
        ...
        :-:|:-:|:-:|:-:|:----
        NO |OFF|OFF|OFF|From  [CARET](../editing/#caret-and-cursor "typing/insertion cursor") location      to  END of file
        YES|OFF|OFF|OFF|From  START of selection  to  END of file
        NO |OFF|ON |OFF|From  START of file       to  [CARET](../editing/#caret-and-cursor "typing/insertion cursor") location
...
In either case, the **Mark All** button will perform the marking.  Similar to [**Replace All**](#find-replace-tabs), **Mark All** will search from the beginning of the document to the end if **Wrap around** is checked; if **Wrap around** is not checked, it will mark from the [caret](../editing/#caret-and-cursor "typing/insertion cursor") position to the end of the file (if **Backward direction** is not checked) or from the beginning of the file to the [caret](../editing/#caret-and-cursor "typing/insertion cursor") position (if **Backward direction** is checked).
...
* **Select and Find Next (<kbd>Ctrl+F3</kbd>) / Select and Find Previous (<kbd>Ctrl+Shift+F3</kbd>)**: Search for the word the [caret](../editing/#caret-and-cursor "typing/insertion cursor") is in, or the selected text, down or up. The searched word or selection is stored in the find history, and the search can be repeated with **Find Next / Find Previous**.
    - The specific search behavior:
        - copies the selected text to **Find what** box of **Find** window for future use, then uses that same string for this specific search
            - if there is no selection and the [caret](../editing/#caret-and-cursor "typing/insertion cursor") is just at a single character position, it uses the word that the [caret](../editing/#caret-and-cursor "typing/insertion cursor") position is a part of (if any)
            - if the [caret](../editing/#caret-and-cursor "typing/insertion cursor") is in whitespace with no selection active, it will not perform a search, and the [caret](../editing/#caret-and-cursor "typing/insertion cursor") will remain where it was
...
* **Find (Volatile) Next (<kbd>Ctrl+Alt+F3</kbd>) / Find (Volatile) Previous (<kbd>Ctrl+Alt+Shift+F3</kbd>)**: Search for the word the [caret](../editing/#caret-and-cursor "typing/insertion cursor") is in, or the selected text, down or up.
    - The searched word or selection is not stored in the find history, and the search will not be repeatable with **Find Next / Find Previous**. ("Volatile" here means "not stored".)  However, because it will have moved the [caret](../editing/#caret-and-cursor "typing/insertion cursor") and selection to the next match, repeated **Find (Volatile) Next / Find (Volatile) Previous** works as expected.
    - The specific search behavior:
...
- Use [possessive quantifiers](#multiplying-operators): for example, try to make matches "greedy" (possessive) using the `+` modifier, so they don't backtrack; or make them "non-greedy" (minimal) using the `?` modifier, so they try to match as _few_ characters as possible, which also reduces backtracking.
...
* `1000000000` = 512 - Backward direction _disabled_ (thus forward direction from [caret](../editing/#caret-and-cursor "typing/insertion cursor") toward bottom end of file)
