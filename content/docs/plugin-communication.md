...
The message IDs for each of these named messages, as well as the enums used with these messages, can be found in the source code in [Notepad_plus_msgs.h](https://github.com/notepad-plus-plus/notepad-plus-plus/blob/master/PowerEditor/src/MISC/PluginsManager/Notepad_plus_msgs.h).  (If you are writing your plugin in C++ or similar languages, you should just include the `Notepad_plus_msgs.h` to get all the constants; but if you are using a different language for your plugin or other message-interface, you will need to translate those #defines to constants or values appropriate to your language.  When reading the #define for the `NPPM_` constants, you need to notice that `NPPMSG` is defined as `(WM_USER + 1000)` and need to know that [`WM_USER`](https://learn.microsoft.com/en-us/windows/win32/winmsg/wm-user) is the Windows standard constant with value `0x0400` (1024), so all the `#define` for messages that are like `(NPPMSG + ##)` mean that you need to add 1024 + 1000 to the `##` given to get the final message ID.  For example, `#define NPPM_SAVEALLFILES (NPPMSG + 39)` is really the integer 1024 + 1000 + 39 = 2063, so that is the value you need when defining your version of the NPPM_SAVEALLFILES constant.  You may need to look up other `#define`d values from elsewhere in the header file in order to fully resolve some of the values.)
